...
window.addEventListener('scroll', () => {
    ...
    // Keep scene.background transparent so body background image always shows through
...
function init3D() {
    ...
    const dnaH  = 10;     // tall helix
    const turns = 1.5;    // 1.5 full rotations top-to-bottom
    const pairs = 40;     // 40 base pair nodes
...
function applyTheme(theme) {
    ...
    if (typeof scene !== 'undefined' && scene) {
        if (theme === 'light') {
            scene.background = null; // Force transparent background to allow hero image to show
